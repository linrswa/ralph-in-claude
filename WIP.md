...
- [ ] **`exit 1` → `exit 2`** — `validate-prd-write.sh` 驗證失敗時用 `exit 1`，但 Claude Code hooks 只有 `exit 2` 才能阻擋操作。`exit 1` 屬於 "any other exit code"，操作仍會繼續執行。需修改 `skills/convert/scripts/validate-prd-write.sh` 和 `skills/run/scripts/validate-prd-write.sh` 中所有 `exit 1` 為 `exit 2`。
- [ ] **`storyIdToTaskId` mapping compaction 遺失** — dispatcher 的 `storyIdToTaskId` 和 `retries` 只存在於 context memory 中。大型 PRD（10+ stories、多波次）會導致 context 接近上限，compaction 後 mapping 消失，調度崩潰。修復方向：(a) 每波持久化到 `.ralph-in-claude/.task-mapping.json`，或 (b) 從 TaskList 的 subject（格式 `US-001: Title`）反建 mapping。
...
- [ ] **同 branch 並行 git race condition** — 多個 subagent 共享同一 git 工作樹，file overlap check 只依賴 story `notes` 中手動提及的檔案路徑，實際修改可能超出預估（shared utils、config 等）。短期：wave 結束後加 rebase/merge check；長期：考慮 per-story git worktree。
...
- [ ] **Write matcher 範圍不足** — 目前只攔截 `Write` tool，但 `Edit` tool 也能修改 prd.json。Matcher 應擴展為同時覆蓋 `Write` 和 `Edit`。
...
- [ ] **Hook 腳本重複** — `convert/scripts/` 和 `run/scripts/` 中的 `ensure-ralph-dir.sh` 和 `validate-prd-write.sh` 完全相同。應共用一份放在 plugin root 的 `scripts/` 目錄。
