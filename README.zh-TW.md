...
- **波次審查：** 每個多 story 波次結束後，執行雙階段審查：**Phase A** 以完整波次上下文解析延遲的合併衝突（Tier 3）——Sonnet reviewer 先行嘗試解析，無法解析時升級至 Opus coordinator；**Phase B** 檢查合併後的 diff 是否有跨 story 一致性問題（命名、import、風格），重大問題升級至 coordinator。
