...
本 repo 自帶 `marketplace.json`，可直接作為獨立市集使用：
...
如果你有維護個人市集 repo（例如 `your-org/claude-plugins`），在你的 `marketplace.json` 中加入 Ralph：
...
兩種方式都會在所有專案中啟用 `/ralph:prd`、`/ralph:convert` 和 `/ralph:run` 指令。
